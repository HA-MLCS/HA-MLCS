...
public class Graph {
    ...
    private Set<Location> heads = new HashSet<Location>(4);//
...
//        collectPartialPath(end, new LinkedList<Location>(), results);
        ...
        Map<Location, Integer> visitedPrecessorMap = new HashMap<>();
        visitedPrecessorMap.put(end.loc, 0);
        queue.addLast(end);
        while (!queue.isEmpty()) {
            Node node = queue.getLast();
            int maxLevel = node.level - 1;
            int visitIndex = visitedPrecessorMap.get(node.loc);
            visitedPrecessorMap.put(node.loc, visitIndex + 1);
            if (visitIndex < node.predecessors.size()) {
                Node p = node.predecessors.get(visitIndex);
                if (!p.predecessors.isEmpty() && p.level == maxLevel) {
                    if (p.predecessors.contains(start)) {
                        List<Location> newPaths = new ArrayList<Location>();
                        newPaths.add(p.loc);
                        for (int i = queue.size() - 1; i > 0; i--) {
                            newPaths.add(queue.get(i).loc);
                        }
                        results.addLast(newPaths);
                        if (results.size() >= 30) break;
                    } else {
                        queue.addLast(p);
                        visitedPrecessorMap.put(p.loc, 0);
                    }
                }
            } else {
                queue.removeLast();
