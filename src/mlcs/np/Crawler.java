...
public class Crawler {
    ...
    public static int reserveThreshold = 10;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (String arg : args) {
            ...
            for (String file : fileNames) {
                ...
                LogWriter.getLogWriter("log-" + file+".log");
                ...
                String removeNumProp = System.getProperty("mlcs.removeThreshold");
                if (null != removeNumProp) {
                    reserveThreshold = Integer.parseInt(removeNumProp);
                    prompts.append(" reserveNumProp=" + reserveThreshold);
                }


                LogWriter.log("Starting search " + file + prompts);
                System.out.println("Starting search " + file + prompts);
                for (int i = 0; i < 1; i++) {
                    System.gc();
                    ...
                    LogWriter.log("\nkeynode num: " + graph.keyLocs.size() + "\n" + result.count + " mlcs(max length " + result.length
                            + ") found\nusing " + Stopwatch.format(System.nanoTime() - start));
...
//					}
                    ...
                    LogWriter.close();
                    System.out.println("Write MLCS result file......");
                    List<List<Location>> paths =graph.findKeyPartialPaths() ;
                    ...
                    Main.writeFile(file + "_reslut", strPaths);
    ...
    public static Mlcs.Result bfs(Mlcs mlcs, Graph graph) {
        ...
        Set<Location> locations = MyBestOrderSort.sortAndSelect(new ArrayList<Location>(new HashSet<>(mlcs.nextSortedLocations(start))), reserveThreshold);
    ...
    private static void search(Mlcs mlcs, Graph graph, List<Location> starts) {
        ...
        ArrayList<Location> locations = new ArrayList<>(starts);
...
//        }
        ...
        int i = 0;
        int threshold = 100;            //After the  number of points in current iteration layer exceeds this threshold, the deletion strategy is adopted.
        boolean delete = true;
        ...
        while (!locations.isEmpty()) {
            ...
            // System.out.println(locations+" "+delete);
            //data segmentation
...
//                newLocationsList = RadixSortingbeta2.sort(newLocationsList, i, 0.5);
                newLocationsList = MyBestOrderSort.sortAndSelect(new ArrayList<Location>(newLocationsList), reserveThreshold);
            }
    ...
    public static void deleteNode(Graph graph) {
        while (!toDelete.isEmpty()) {
            ...
            for (Graph.Node n : graph.location2Nodes.get(node).predecessors) {
